...
class AddEditLead extends Component {
        ...
        if ((gonext && formsubmitted) || gonosubmitnext) {
            ...
            if (navigation_state.items[nextindex]) {
                this.setState({
                    ...
                    formsubmitted: false,
                    lead_id: navigation_state.items[nextindex].id
                }, () => {
                    this.props.history.push(this.props.location.pathname + "?lead_id=" + navigation_state.items[nextindex].id);
                    this.fetchRecord();
                })
            } else {
        ...
        if ((goprev && formsubmitted) || gonosubmitprev) {
            ...
            if (navigation_state.items[previndex]) {
                this.setState({
                    ...
                    formsubmitted: false,
                    lead_id: navigation_state.items[previndex].id
                }, () => {
                    this.props.history.push(this.props.location.pathname + "?lead_id=" + navigation_state.items[previndex].id);
                    this.fetchRecord();
                })
            }
        }
        if ((formsubmitted && currentPage)) {
            ...
            if (navigation_state.items[currentindex]) {
                this.setState({
                    ...
                    gonosubmitnext: false,
                    formsubmitted: false,
                    lead_id: navigation_state.items[currentindex].id
                }, () => {
                    this.props.history.push(this.props.location.pathname + "?lead_id=" + navigation_state.items[currentindex].id);
                    this.fetchRecord();
    ...
    fetchLayout() {
        ...
        if (!fields) {
            fetch(`${window.location.protocol}://${window.location.hostname}:${process.env.REACT_APP_PORT}/api/leads/leadsaddeditlayout/?page_size=100`, {
                method: 'GET',
    ...
    fetchRecord() {
        ...
        if (this.state.lead_id && this.state.navigation_state) {


            this.state.navigation_state.items.map((item, i) => {

                if (this.state.lead_id == item.id) {
                    fetch(`${window.location.protocol}://${window.location.hostname}:${process.env.REACT_APP_PORT}/api/leads/` + this.state.lead_id + '/', {
                        method: 'GET',
                        headers: { 'Content-Type': 'application/json', 'Authorization': 'Token ' + sessionStorage.getItem("authUser").replace(/^"(.*)"$/, '$1') }
                    })
                        .then(res => res.json())
    ...
    generateColumns(rowNumber) {
        const { lead_id, isLoaded, fields } = this.state;
    ...
    generateLayout() {
        ...
        if (isLoaded) {
            ...
            fields && fields.length > 0 && fields.map((field, index) => {
                if ((!lead_id && field.layout_field_display_add_screen) || lead_id) {
    ...
    handleSubmit = (event, errors, values) => {
        ...
        const { lead_id, record, navigation_state, currentindex, nextindex, previndex, gonext, data, submit } = this.state;
        ...
        return new Promise((resolve, reject) => {
            fetch(`${window.location.protocol}://${window.location.hostname}:${process.env.REACT_APP_PORT}/api/leads/` + lead_id + '/', {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json', 'Authorization': 'Token ' + sessionStorage.getItem("authUser").replace(/^"(.*)"$/, '$1') },
        ...
        fetch(navigation_state.prevpage, {
            ...
            .then(data => {
                ...
                this.setState({
                    ...
                    lead_id: data.results[parseInt(data.page_size) - 1].id,
                    gonosubmitprev: true,
                    gonosubmitnext: false,
                    showSuccessAlert: false,
                    showFailureAlert: false,
    ...
    render() {
        const { lead_id, isLoaded, fields, data, navigation_state, nextindex, previndex, currentindex, redirectToLeads, showSuccessAlert, showFailureAlert, showFailureMessage } = this.state;
            ...
            <React.Fragment>
                ...
                <Col sm={6}>
                    ...
                    <ol className="breadcrumb mb-0">
                        ...
                        <li className="breadcrumb-item active">{lead_id ? 'Edit Lead' : 'Add Lead'}</li>
                    </ol>
                    </div>
                </Col>
                </Row>
