...
class Leads extends Component{
  ...
  fetchData() {
    ...
    fetch(`${process.env.REACT_APP_API_URL}:${process.env.REACT_APP_PORT}/api/leads${qs}`, {
      method: 'GET',
      headers: { 'Content-Type': 'application/json', 'Authorization':'Token '+sessionStorage.getItem("authUser").replace(/^"(.*)"$/, '$1')}
    })
    .then(res => res.json())
  ...
  deleteSelected = () => {
    ...
    fetch(`${process.env.REACT_APP_API_URL}:${process.env.REACT_APP_PORT}/api/leads/delete-leads/`, {
      method: 'DELETE',
      headers: { 'Content-Type': 'application/json', 'Authorization':'Token '+sessionStorage.getItem("authUser").replace(/^"(.*)"$/, '$1')},
